 ...
 */
public class LoadGraphAction2 extends Action {
    ...
    protected ElkNode loadAndLayout(String filePath) {
        ...
        try {
            ...
            // TODO : Insert "_arrange" before the existing extension
            URI uriArranged = URI.createFileURI(filePath.concat("_arranged.elkg"));
            Resource resourceArranged = resourceSet.createResource(uriArranged);
            content.setProperty(CoreOptions.NO_LAYOUT, true);
            resourceArranged.getContents().add(content);
            try {
                resourceArranged.save(Collections.emptyMap());
            } catch (IOException e) {
                // ignore the exception
            }
            content.setProperty(CoreOptions.NO_LAYOUT, false);
